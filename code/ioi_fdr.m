function q=ioi_fdr(p,dim)
% IOI_FDR False discovery rate
% Q=IOI_FDR(P); returns vector Q of estimated false discovery rates (set-level q-values) from 
% a vector P of multiple-test false positive levels (uncorrected p-values)
% Q=IOI_FDR(P,dim); where P is a matrix computes the fdr along the dimension dim of P
%
