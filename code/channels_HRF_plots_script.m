...
hHbOL=figure; set(hHbOL, 'color', 'w', 'Name', 'HbO L condition')
myYlims = [-30 100];
...
hHbOR=figure; set(hHbOR, 'color', 'w', 'Name', 'HbO R condition')
for iChannels=1:nChannels+1
    subplot(nRow, nCol, iChannels)
    hold on
    if iChannels<=nChannels
...
% Change figure and paper size
set(hHbOR, 'Position', [0.1 0.1 15 10])
set(hHbOR, 'PaperPosition', [0.1 0.1 15 10])
if printFigs
...
if printFigs
    print(hHbRL, '-dpng', '..\figures\HbR_L_group_avg.png', sprintf('-r%d',300));
end
...
if printFigs
    print(hHbRR, '-dpng', '..\figures\HbR_R_group_avg.png', sprintf('-r%d',300));
end
