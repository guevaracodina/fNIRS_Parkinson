...
controlDir = '..\fNIRS_Data\Parkinson\resting_state\Control\derivatives\homer';
ParkinsonDir = '..\fNIRS_Data\Parkinson\resting_state\PD\derivatives\homer';

for iHb = 1:3               % 1=HbO, 2=HbR, 3=HbT
    % Control Group
    [control.rMatFDR, control.zMatFDR, control.pMatFDR, control.pMat, control.keepRun] = ...
        conn_mat_group_level(controlDir, iHb, 'controlConn');
    % Parkinson Group
    [PD.rMatFDR, PD.zMatFDR, PD.pMatFDR, PD.pMat, PD.keepRun] = ...
        conn_mat_group_level(ParkinsonDir, iHb, 'PDConn');
    % Get Hb string label
    strContrast = get_Hb_string(iHb);
    % Save all fc data
    save(fullfile('..\data\', ['all_significant_connections_' strContrast '.mat']));
end
