% Computes correlations about the following HRF metrics on a channel-by-channel
    ...
    for idxDemo = idxVars
        ...
        if isa(allDemographics.(variableNames{idxDemo}), 'double')
            ...
            if pValsHbO(idxHRF, idxDemo) < 0.05
                ...
                % Print figures
                if printFigures
                    figName = strcat('..\figures\correlations\', 'HbO_corr_5sec_', fNIRSvarsNames{idxHRF}, '_', variableNames{idxDemo}, '.png');
    ...
    for idxDemo = idxVars
        ...
        if isa(allDemographics.(variableNames{idxDemo}), 'double')
            ...
            if pValsHbR(idxHRF, idxDemo) < 0.05
                ...
                % Print figures
                if printFigures
                    figName = strcat('..\figures\correlations\', 'HbR_corr_5sec_', fNIRSvarsNames{idxHRF}, '_', variableNames{idxDemo}, '.png');
                    print(hPlot, '-dpng',  figName , sprintf('-r%d',300));
                end
            end
        end
    ...
    for idxDemo = idxVars
        ...
        if isa(allDemographics.(variableNames{idxDemo}), 'double')
            ...
            if pValsHbT(idxHRF, idxDemo) < 0.05
                ...
                % Print figures
                if printFigures
                    figName = strcat('..\figures\correlations\', 'HbT_corr_5sec_', fNIRSvarsNames{idxHRF}, '_', variableNames{idxDemo}, '.png');
                    print(hPlot, '-dpng',  figName , sprintf('-r%d',300));
                end
            end
        end
...
set(hCorr, 'color', 'w')
if printFigures
    print(hCorr, '-dpng', '..\figures\correlations\corr_matrix_5sec.png', sprintf('-r%d',300));
end
% save ('..\fNIRS_Data\corrData_5sec.mat')
% EOF